...
// ⚠️ COLOQUE SUAS CREDENCIAIS REAIS AQUI ANTES DE SALVAR
...
// ==========================================
// PAGAMENTO COM CARTÃO (INJEÇÃO DA ETIQUETA CPF)
// ==========================================
...
async function processarPagamentoCartao(formData, resolve, reject) {
    ...
    feedback.innerHTML = "⏳ Processando cartão de forma segura...";

    try {
        // A MÁGICA FINAL: Garantir que o tipo de documento vá como "CPF" para o banco aceitar
        let tipoDocumento = "CPF";
        let numeroDocumento = "";
        
        if (formData.payer && formData.payer.identification) {
            if (formData.payer.identification.type) tipoDocumento = formData.payer.identification.type;
            if (formData.payer.identification.number) numeroDocumento = formData.payer.identification.number;
        }
        ...
        const dadosDoPagamento = {
            ...
            payer: {
                ...
                identification: {
                    type: tipoDocumento, // A etiqueta que o banco estava a exigir!
        ...
        feedback.innerHTML = "❌ Falha na conexão com o Mercado Pago.";
        reject();
    }
}
