...
document.getElementById('btn-show-card').addEventListener('click', async () => {
    try {
        Swal.fire({ title: 'Gerando Assinatura...', text: 'Conectando ao sistema seguro...', background: '#161618', color: '#fff', didOpen: () => { Swal.showLoading() } });

        // 🔥 ATENÇÃO MESTRE: COLE O SEU ID DO PLANO AQUI ENTRE AS ASPAS!
        const ID_DO_PLANO = "https://www.mercadopago.com.br/subscriptions/checkout?preapproval_plan_id=867728a336404feca158d63874ccea3b"; 
        
        // Pega o email do aluno (usando um padrão caso ele não tenha cadastrado um)
        const emailAluno = "aluno_" + Math.floor(Math.random() * 1000) + "@4lacademy.com";

        // Chama a nossa Edge Function no Supabase
        const { data, error } = await supabase.functions.invoke('gerar-assinatura', {
            body: { email: emailAluno, plan_id: ID_DO_PLANO }
        });

        if (error || !data) throw new Error("Falha na comunicação com o servidor.");

        // O Mercado Pago devolve um link de pagamento (init_point)
        if (data.init_point) {
            Swal.close();
            // Redireciona o aluno para a tela oficial de Assinatura do Mercado Pago
            window.location.href = data.init_point;
        } else {
            // 🔥 O VAR: Mostra o texto exato que o Mercado Pago devolveu
            let erroReal = data.message || data.error || JSON.stringify(data);
            Swal.fire({ icon: 'error', title: 'Recusado pelo MP', text: `Motivo: ${erroReal}`, background: '#161618', color: '#fff' });
            console.error("Retorno do MP:", data);
        }

        
    } catch (err) {
        Swal.fire({ icon: 'error', title: 'Erro de Conexão', text: err.message, background: '#161618', color: '#fff' });
        console.error("Falha ao gerar assinatura:", err);
    }
});
...
// ==========================================
// 📸 GESTÃO DA FOTO DE PERFIL (ImgBB)
// ==========================================
const IMGBB_API_KEY = "e70fa8d1a3a16d5cd774751805b617af"; 
...
document.getElementById('input-foto').addEventListener('change', async (e) => {
    ...
    try {
        ...
        const dadosImgBB = await res.json();

        if (dadosImgBB.success) {
            const linkDaFoto = dadosImgBB.data.url;
        ...
        Swal.fire({ icon: 'error', title: 'Falha no Upload', text: 'Tente novamente mais tarde.', background: '#161618', color: '#fff' });
    }
});
