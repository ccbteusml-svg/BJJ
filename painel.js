...
async function verificarAcesso() {
    ...
    if (mensalidades && mensalidades.length > 0) {
        ...
        // Se tiver PIX pendente, verifica no Mercado Pago se já caiu
...
// ==========================================
// PAGAMENTO COM PIX (COPIA E COLA) BLINDADO 🛡️
// ==========================================
document.getElementById('btn-pagar').addEventListener('click', async () => {
    ...
    try {
        // 🔥 Chama a função segura na nuvem
...
// ==========================================
const IMGBB_API_KEY = "e70fa8d1a3a16d5cd774751805b617af"; // Pegue sua chave em https://api.imgbb.com/
...
document.getElementById('input-foto').addEventListener('change', async (e) => {
    ...
    // 2. Preparar a imagem para o ImgBB
    const formData = new FormData();
    formData.append("image", arquivo);

    try {
        // 3. Upload para o ImgBB
        ...
        if (dadosImgBB.success) {
            ...
            // 4. Salvar o link no perfil do aluno no Supabase
            const { data: { session } } = await supabase.auth.getSession();
...
// Atualizar a função verificarAcesso() para carregar a foto ao abrir o app
// Dentro da função verificarAcesso(), adicione isto onde você já busca o nome e faixa:
/*
    if (perfil.foto_url) {
