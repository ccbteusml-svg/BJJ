...
// ==========================================
// ABRIR MAQUININHA DO CARTÃO (GATILHO)
// ==========================================
document.getElementById('btn-show-card').addEventListener('click', async () => {
    try {
        ...
        const valorTexto = document.getElementById('valor-pagamento').innerText.replace('R$ ', '').replace(',', '.').trim();
        ...
        const settings = {
            ...
            callbacks: {
                ...
                // 🔥 A CORREÇÃO ESTÁ AQUI: Removemos as chaves { } em volta do formData
                onSubmit: (formData) => {
                ...
                onError: (error) => { 
                    Swal.fire({ icon: 'error', title: 'Erro do Banco', text: 'Não foi possível carregar a máquina.', background: '#161618', color: '#fff' });
        ...
        Swal.fire({ icon: 'error', title: 'Erro no Layout', text: 'A div cardPaymentBrick_container desapareceu do HTML.', background: '#161618', color: '#fff' });
        console.error("Falha ao abrir cartão:", err);
    }
});
...
async function processarPagamentoCartao(formData, resolve, reject) {
    ...
    try {
        ...
        if (formData.payer) {
            if (formData.payer.email) emailAluno = formData.payer.email;
            if (formData.payer.identification) {
                if (formData.payer.identification.type) docTipo = formData.payer.identification.type;
                if (formData.payer.identification.number) docNumero = formData.payer.identification.number;
            }
        }
        ...
        Swal.fire({ icon: 'error', title: 'Falha no Celular', text: `Motivo Exato: ${e.message}`, background: '#161618', color: '#fff', confirmButtonColor: '#E53935' });
