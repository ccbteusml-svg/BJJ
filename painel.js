...
document.getElementById('btn-show-card').addEventListener('click', async () => {
    try {
        ...
        // 👇 A MÁGICA VISUAL ACONTECE AQUI NESTA VARIÁVEL "settings" 👇
        const settings = {
            initialization: { amount: 25 }, // Valor da sua assinatura
            customization: { 
                visual: { 
                    style: { theme: 'dark' },
                    texts: {
                        formTitle: "Cartão de crédito" // 🔥 Muda o título
                    }
                }, 
                paymentMethods: { 
                    maxInstallments: 1,
                    types: { excluded: ['debit_card'] } // 🔥 Bloqueia cartão de débito
                } 
            },
            callbacks: {
                onReady: () => { Swal.close(); },
                onSubmit: (dadosRecebidos) => {
                    return new Promise(async (resolve, reject) => {
                        ...
                        try {
                            ...
                            const cardToken = form.token; // Pega o token seguro do cartão
                            ...
                            // Verifica se o Mercado Pago aprovou
