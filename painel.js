// ==========================================
// 🛡️ SISTEMA DE DEFESA: MODO MANUTENÇÃO
// ==========================================
async function verificarManutencaoPainel() {
    try {
        // 1. Pergunta ao banco se o Sensei ligou a manutenção
        ...
        if (config && config.manutencao_ativa) {
            // 2. Se a manutenção estiver ligada, descobre quem está tentando entrar
            ...
            // 3. Se NÃO for o professor, jogamos a "Cortina" por cima de tudo
            if (!isProfessor) {
                ...
                // Adiciona a cortina na tela SEM apagar o HTML original
                document.body.appendChild(cortinaManutencao);
                document.body.style.overflow = 'hidden'; // Trava o scroll do aluno
            }
        }
    } catch (error) {
...
// Executa a defesa imediatamente ao carregar o aplicativo
