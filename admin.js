...
async function carregarTodosAlunos() {
    ...
    // Conta quantos alunos existem em cada faixa (AGRUPANDO POR COR)
    alunosAtivos.forEach(aluno => {
        let faixaCompleta = aluno.faixa || 'Branca';
        
        // 1. Corta tudo que vem depois da barra (/) ou traço (-)
        let corDaFaixa = faixaCompleta.split('/')[0].split('-')[0].trim();
        
        // 2. Remove a palavra "Faixa " (se existir) para economizar espaço nas pílulas
        corDaFaixa = corDaFaixa.replace(/faixa/i, '').trim();
        
        // 3. Garante que a primeira letra fique maiúscula (ex: "azul" -> "Azul")
        corDaFaixa = corDaFaixa.charAt(0).toUpperCase() + corDaFaixa.slice(1).toLowerCase();
        
        if(!corDaFaixa) corDaFaixa = 'Branca';

        // 4. Faz a contagem usando APENAS a cor base
...
window.toggleSanfona = function(idSecao) {
    ...
    const seta = document.getElementById('seta-' + idSecao);

    if (conteudo.style.display === 'none' || conteudo.style.display === '') {
        conteudo.style.display = 'block';
        seta.innerText = '▲'; // Seta para cima quando abre
    } else {
        conteudo.style.display = 'none';
        seta.innerText = '▼'; // Seta para baixo quando fecha
    }
};
...
document.getElementById('btn-disparar-robos').addEventListener('click', async (e) => {
                ...
                try {
                    // CORRIGIDO AQUI: O nome exato da sua Edge Function
                    const { data, error } = await supabase.functions.invoke('robo-cobranca');
                    if (error) throw error;
...
// ==========================================
// CONTROLE DO MODO MANUTENÇÃO (CORRIGIDO)
// ==========================================

// 1. Função para trocar o status (Liga/Desliga)
window.alterarModoManutencao = async function() {
    const btn = document.getElementById('btn-toggle-manutencao');
    
    // USANDO 'supabase' (sem o underline) para bater com o seu arquivo
    ...
    const novoStatus = !data.manutencao_ativa;

    // Atualiza o banco com o novo status usando o nome correto 'supabase'
    const { error: updateError } = await supabase
        .from('sistema_config')
        .update({ manutencao_ativa: novoStatus })
        .eq('id', 1);

    if (updateError) {
        alert("Erro ao atualizar o Modo Manutenção!");
    } else {
        atualizarVisualBotao(novoStatus);
        Swal.fire({
            icon: 'success',
            title: novoStatus ? "🚩 BLOQUEIO ATIVADO!" : "✅ BLOQUEIO DESATIVADO!",
            background: '#161618',
            color: '#fff',
            showConfirmButton: false,
            timer: 2000
...
// 2. Função para mudar a cor e o texto do botão na tela
function atualizarVisualBotao(ativa) {
    const btn = document.getElementById('btn-toggle-manutencao');
    if (!btn) return;

    if (ativa) {
        btn.innerText = "DESATIVAR BLOQUEIO";
        btn.style.background = "#4CAF50"; // Verde (próxima ação é desligar)
    } else {
        btn.innerText = "ATIVAR BLOQUEIO";
        btn.style.background = "#e53935"; // Vermelho (próxima ação é bloquear)
    }
}

// 3. Checa o status assim que você abre o painel Admin
async function checarStatusInicial() {
    // Novamente removendo o underline para usar 'supabase'
    ...
    if (data) {
        atualizarVisualBotao(data.manutencao_ativa);
    }
}
